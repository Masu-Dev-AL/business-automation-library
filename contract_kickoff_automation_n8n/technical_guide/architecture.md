...
flowchart LR
    ...
    subgraph WorkflowA["⚙️ n8n Workflow A — Contract Ingestion"]
        ...
        A3["A3. Download PDF"]
        A4["A4. Claude Analyze Document"]
        A5b["A5b. Parse Response"]
        A6["A6. Quality Score"]
        A7{{"A7. Quality Check"}}
    ...
    class DS trigger
    class A1,A2,A3,A5b,A6,A8,A9,A10,A12,A13,A14 node
    class A4,A11 ai
    class A7 decision
    class A15 alert
...
| Stage | Input | Output |
|-------|-------|--------|
| Trigger (A1–A2) | DocuSign Connect envelope payload | `document_id`, `client_name`, `client_email` |
| Ingestion (A3–A4) | DocuSign envelope ID | Structured JSON extracted by Claude from PDF |
| Parsing (A5b–A6) | Claude response | Validated JSON: deliverables, dates, quality score |
...
- **Native Anthropic node** — Claude reads the PDF binary directly via the n8n Anthropic node; no pdfminer or text extraction sidecar required
